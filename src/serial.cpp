how about thos serial #include "body.hpp"
...
int main() {
  ...
  // Optional: guard
  if (dt_step <= 0.0) {
    // we're exactly at an output boundary or end; advance output boundary
  ...
  const double dt_raw =
    compute_local_dt<DIM>(bodies, accumulator, dt_step,
                          1e100, dt_min, eps_v, v_floor, a_floor, 1e100);
