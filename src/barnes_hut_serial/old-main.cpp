...
int main(int argc, char* argv[]) {
    ...
    int steps;
    double dt;
    bodies<DIM, EmptyAttributes> bodiesFromFile;

    utils::generateRandomToFile<DIM>("test1.in.out", 1000, 1000, 0.01, 42);
    utils::readFromFile("test1.in.out", steps, dt, bodiesFromFile);
    n = bodiesFromFile.globalSize();
    ...
    for (int i = 0; i < n; i++) {
        ...
        bodies[i].force = 0.;
        bodies[i].mass = bodiesFromFile.global(i).mass();
        bodies[i].position = bodiesFromFile.global(i).pos();
        bodies[i].velocity = bodiesFromFile.global(i).vel();
    }
