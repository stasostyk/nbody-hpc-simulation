...
void updateParticles(int index, bodies<DIM>& bodies, std::vector<glm::vec3>& combinedTraces) {
    ...
    int n;
    double total_time;
    utils::readFromFile(FILE_PATH+std::to_string(index)+".out", n, total_time, dt, bodies, false);
    n = static_cast<int>(bodies.position.size());
...
int main() {
    ...
    utils::readFromFile(FILE_PATH + "0.out", n, total_time, dt, bodies, false);
    n = static_cast<int>(bodies.position.size());
