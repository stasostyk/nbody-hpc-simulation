...
inline void saveToFile(
    ...
    // Output in a single file, exactly as requested
    fout << DIM << "\n";
    fout << n << " " << steps << " " << dt << "\n";
    for (int i = 0; i < n; i++) {
        fout << m[i] << " ";
...
//
// readFromFile MODIFICATO SOLO PER LEGGERE IL TUO input.txt
//
// formato:
//
// n steps dt
...
inline void readFromFile(
    ...
    fin.close();
    std::cout << "Read from file " << fileName << std::endl;
    std::cout << "  n=" << n << " steps=" << steps << " dt=" << dt << std::endl;
}

/*
// COMMENTATA come richiesto
inline void initAndSaveRandom() {
    ...
}
*/
...
inline void runSerial() {
    ...
    // LEGGE IL TUO FILE
    readFromFile("input.txt", n, steps, dt, masses, positions, velocities);
    ...
    // SALVA TUTTO NEL FILE RICHIESTO
    saveToFile("output_base.txt", n, steps, dt, masses, positions, velocities);
}
